...
public final class ActionScript3Resolver {
	...
	private static Map<MethodInfo, ResolvedMethod> createMethodInformationMapping(final AS3Data data, final List<String> errors) {
		...
		for (final MethodBody methodBody : methodBodies) {
			...
			if (methodIndex >= methodInfos.size()) {
				...
				final ResolvedMethod resolvedMethod = resolveMethod(methodInfo, code, data.getConstantPool(), errors);
		...
		for (final MethodInfo methodInfo : data.getMethodInfos()) {
			...
			final ResolvedMethod resolvedMethod = resolveMethod(methodInfo, null, data.getConstantPool(), errors);
			methodMapping.put(methodInfo, resolvedMethod);
		}
	...
	private static ResolvedClass resolveClass(final AS3Data data, final ClassInfo classInfo, final InstanceInfo instanceInfo, final Map<MethodInfo, ResolvedMethod> methodMapping, final List<String> errors) {
		...
		final String[] className = resolveClassName(instanceInfo, data.getConstantPool(), errors);
		final List<ResolvedMethod> methods = resolveMethods(instanceInfo.getTraits(), methodList, methodMapping, constantPool, errors);
	 ...
	 */
	private static String[] resolveClassName(final InstanceInfo instanceInfo, final ConstantPool constantPool, final List<String> errors) {

		assert instanceInfo != null : "Instance information argument must not be null";
		assert errors != null : "Errors list must not be null";

		final MultinameInfoList multinameList = constantPool.getMultinames();
		...
		if (classNameIndex == 0) {
			...
			try {
				return ActionScript3Helpers.resolveMultiname(multinameInfo, constantPool);
	 ...
	 */
	private static ResolvedMethod resolveMethod(final MethodInfo methodInfo, final AS3Code code, final ConstantPool constantPool, final List<String> errors) {

		assert methodInfo != null : "Method information argument must not be null";
		assert errors != null : "Errors list must not be null";

		final String name = resolveMethodName(methodInfo, constantPool, errors);
		final String[] returnType = resolveMethodReturnType(methodInfo, constantPool, errors);
		final List<String[]> arguments = resolveMethodArguments(methodInfo, constantPool, errors);
	...
	private static ResolvedMethod resolveMethod(final TraitsInfo traitsInfo,
		...
		final String[] methodName = resolveMethodName(traitsInfo, constantPool, errors);
	 ...
	 */
	private static List<String[]> resolveMethodArguments(final MethodInfo methodInfo, final ConstantPool constantPool, final List<String> errors) {
		...
		for (int i=0;i<methodInfo.getParamTypes().size();i++) {
			...
			if (paramTypeIndex == 0) {
				...
				try {
					final String[] name = ActionScript3Helpers.resolveMultiname(multinameInfo, constantPool);
					arguments.add(name);
				} catch (final ResolverException e) {
					errors.add(String.format("Could not resolve argument of method at offset %08X because name index is out of bounds", methodInfo.getBitPosition() / 8));
				}
	 ...
	 */
	private static String resolveMethodName(final MethodInfo methodInfo, final ConstantPool constantPool, final List<String> errors) {

		assert methodInfo != null : "Method information argument must not be null";

		final StringInfoList constantStringList = constantPool.getStrings();
	 ...
	 */
	private static String[] resolveMethodName(final TraitsInfo traitsInfo, final ConstantPool constantPool, final List<String> errors) {
	 ...
	 */
	private static String[] resolveMethodReturnType(final MethodInfo methodInfo, final ConstantPool constantPool, final List<String> errors) {

		assert methodInfo != null : "Method information argument must not be null";
		assert errors != null : "Errors list must not be null";

		MultinameInfoList multinameList = constantPool.getMultinames();
	 ...
	 */
	private static List<ResolvedMethod> resolveMethods(final TraitsInfoList traits, final MethodInfoList methodList, final Map<MethodInfo, ResolvedMethod> methodMapping, final ConstantPool constantPool, final List<String> errors) {
		...
		for (final TraitsInfo traitsInfo : traits) {
			if (traitsInfo.getData() instanceof TraitMethod) {
				methods.add(resolveMethod(traitsInfo, methodList, methodMapping, constantPool, errors));
			}
		}
